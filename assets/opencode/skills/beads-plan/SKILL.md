...
3. Clarify before decomposing.
    - List assumptions explicitly before producing any tasks.
    - Identify ambiguities in the request and resolve each one conservatively.
    - Name the modules and layers affected by the change in one or two sentences.
    - When the request is complex (spans 3+ layers, is highly ambiguous, or will likely need more than 15 tasks), interview the user with up to 10 focused questions before proceeding. Do not decompose until the answers are clear.
    - When the request is straightforward, proceed directly after listing assumptions. Do not interview unnecessarily.

4. Turn the request into a concrete plan before creating any beads.
...
6. Validate the plan.
    ...
    - No prohibited placeholders in any field: `TODO`, `TBD`, `FIXME`, `as needed`, `etc.`, `appropriate`, `suitable`, `the relevant files`.
    - Every `description` must include concrete file paths, not generic references.
    - Every `done` criterion must be verifiable by a specific command, assertion, or observable outcome.
